...
async function load (membershipid, ignoreDeletedAccounts) {
...
async function loadMany (membershipids, ignoreDeletedAccounts) {
  if (!membershipids || !membershipids.length) {
    return
  }
  const memberships = []
  for (let i = 0, len = membershipids.length; i < len; i++) {
    const membership = await load(membershipids[i], ignoreDeletedAccounts)
    if (!membership) {
      continue
    }
